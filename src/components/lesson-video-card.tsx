    ...
    <>
      <div className="p-6 rounded-2xl border border-border bg-background hover:border-brand-accent transition-colors flex flex-col">
        ...
        <div className="text-xs text-muted-foreground mt-1">
          {topic.videos.length} lessons · 12 min
        </div>
        <p className="text-xs text-muted-foreground mt-3 leading-relaxed flex-1">{topic.blurb}</p>
        <button
          type="button"
        ...
        >
          <PlayCircle className="size-4" /> Open video
        </button>
      </div>
      ...
      {open && (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label={`${topic.title} lessons`}
            ...
            <div className="flex items-start justify-between gap-4 p-5 border-b border-border">
              <div>
                <div className="text-xs font-bold uppercase tracking-wider text-brand-accent">
                  {topic.title} · Lesson {active + 1} of {topic.videos.length}
                </div>
                <h3 className="text-lg font-bold mt-1">{video.title}</h3>
                <p className="text-xs text-muted-foreground mt-0.5">Source: {video.source}</p>
              </div>
              <button
                type="button"
                onClick={() => setOpen(false)}
